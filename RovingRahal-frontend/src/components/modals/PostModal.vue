<template>
  ...
  >
    <div class="relative w-full max-w-2xl max-h-full">
      ...
      <div class="relative bg-dark rounded-lg shadow dark:bg-gray-700">
        <!-- Modal header -->
        <div
          class="flex items-start justify-between p-4 border-b rounded-t dark:border-slate-700"
        >
        ...
        <div class="p-6 space-y-6 relative">
          <div class="absolute flex">
            <div
              class="rounded-full me-4 flex justify-center items-center bg-white image"
            >
              <img
                src="../../assets/images/profile_test.png"
                class="h-full"
                alt=""
              />
            </div>
            <div>
              ...
              <button ref="btnShowVisibility"
                class="text flex items-center bg-grey-400 privacyBtn rounded-full" @click="showVisibility"
              >
                <svg
                  class="fill-white me-2"
                  xmlns="http://www.w3.org/2000/svg"
                  id="Layer_1"
                  data-name="Layer 1"
                  viewBox="0 0 24 24"
              ...
              <div class="visibilite-chose bg-white" :style="{display: displayDiv}" ref="choseVisibility">
                <div @click="selectOption('public')" class="chose flex justify-center gap-2 w-full h-18 p-2">
                  <svg
                    class="svg-chose"
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                <div @click="selectOption('freinds')" class="chose flex justify-center gap-2 w-full h-18 p-2">
                  <svg class="svg-chose" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div @click="selectOption('private')" class="chose flex justify-center items-center gap-2 w-full h-18 p-2">
                  <svg class="svg-chose" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <textarea
            ...
            placeholder="What's in your mind, Pew Pew?"
            rows="8"
          ></textarea>
          <div
            class="flex flex-col emojis-container absolute right-6 bottom-9 bg-red" ref="divEmoji"
          >
            <div
              class="w-full flex justify-around category-symbols-container border-b"
            >
              <span 
                v-for="(category, index) in emojis"
            ...
            <div class="emoji-category-container h-full flex p-4">
              <span
                v-for="(emoji, index) in category_to_display"
                class="flex justify-center align-center p-2"
                :key="index"
                @click="post_content += emoji"
                >{{ emoji }}</span
              >
            </div>
          </div>
          <button @click="emojisToggle()" ref="btnEmoji">
            <svg
              class="absolute right-6 bottom-0 emojis-toggler"
              xmlns="http://www.w3.org/2000/svg"
              id="Layer_1"
              data-name="Layer 1"
              viewBox="0 0 24 24"
        ...
        <div class="flex items-center p-6 space-x-2 rounded-b justify-between">
          <div
            class="flex items-center justify-between w-full px-4 py-2 rounded-full border"
          >
          ...
          <button
            class="text-white ml-12 px-12 text-lg py-2 brand-background font-bold rounded-full"
          >
            Post
          </button>
        </div>
  ...
  <!-- picutre modal -->
<div id="pictureModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-start justify-between p-4 pb-2 border-b rounded-t dark:border-gray-600">
              <div class="pictureDowload flex justify-center items-center gap-8 ">
                 ...
                 :class="uploadVisible?'notClicked':'isClicked'"
                  @click=" showUploadPicture(false)">Upload picture</span>
                 <span class="upppercase text-md text-bold cursor-pointer" 
                 :class="!uploadVisible ?'notClicked':'isClicked'"
                 ref="menuTakePicture" 
                 @click=" showUploadPicture(true)">Take picture</span>
                </div>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="pictureModal">
            ...
            <div class="p-6 space-y-6 text-center flex flex-col items-center justify-center" style="height: 335px;">
              ...
              <button v-show="!uploadVisible" @click="openFileInput"
    ...
    <img v-if="photoTaken" :src="previewImage" style="max-width: 100%; height: auto;">
            ...
            <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                <button data-modal-hide="pictureModal" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Upload</button>
                <button data-modal-hide="pictureModal" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">cancel</button>
            </div>
        </div>
    </div>
</div>
...
export default {
  ...
  methods: {
    ...
    onClickOutside(referenceBtn,referenceDiv,event){
      const targetElement = event.target;
      const divElement = this.$refs[referenceDiv];
      const buttonElement = this.$refs[referenceBtn];
      if(divElement && 
  ...
  mounted() {
    ...
    document.addEventListener('click',this.onClickOutside.bind(this, 'btnShowVisibility', 'choseVisibility')),
